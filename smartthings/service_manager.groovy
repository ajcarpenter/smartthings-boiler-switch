...
preferences {
	page(name: "searchTargetSelection", title: "UPnP Search Target", nextPage: "deviceDiscovery") {
		section("Search Target") {
			input "searchTarget", "string", title: "Search Target", defaultValue: "urn:schemas-upnp-org:device:BoilerSwitch:1", required: true
		}
	}
...
Map verifiedDevices() {
	...
	def map = [:]
	devices.each {
		def value = it.value.name ?: "Boiler Switch ${it.value.ssdpUSN.split(':')[1][-3..-1]}"
		def key = it.value.mac
		map["${key}"] = value
	}
	map
...
def addDevices() {
	...
	selectedDevices.each { dni ->
		...
		if (!d) {
			log.debug "Creating Boiler Switch Device with dni: ${selectedDevice.value.mac}"
			addChildDevice("ajcarpenter", "Boiler Switch", selectedDevice.value.mac, selectedDevice?.value.hub, [
				"label": selectedDevice?.value?.name ?: "Boiler Switch",
				"data": [
